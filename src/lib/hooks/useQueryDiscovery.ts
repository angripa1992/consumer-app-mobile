...
export const usePostDiscoverySpots = (
	...
	const { data, isError, error, isLoading, refetch } = useQuery({
		queryKey: ['spotsDiscovery', search_content, city, area],
		queryFn: () => {
			setIsLoading(isLoading);
			return fetchPostDiscoverySpots(userInfo, city, area, values, 0);
		},
		onSuccess: (data) => {
			setIsLoading(false);
		},
...
export const usePostDiscoverySpotsWithInfiniteScroll = (
	...
	const {
		...
		queryKey: ['allSpotsDiscovery', search_content, city, area],
		queryFn: async ({ pageParam }) => {
			setIsLoading(isLoading);
			return fetchPostDiscoverySpots(userInfo, city, area, values, pageParam);
		},
